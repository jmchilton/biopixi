...
A recipe naming its package through `${{ ... }}` references to its own `context:` block — the
conventional rattler-build form — is read rather than rejected. A `git=` or `url=` requirement is
now rejected at every depth, not only in the graded manifest, and only tables effective on a
declared platform participate.

Conformant path dependencies are reported on the result as `pathDependencies` and in the JSON
report, each carrying the `scope` it was declared at. A `build.skip` expression produces a lint,
and a lock whose source record names a different directory is reported as `STALE` rather than as
an out-of-profile manifest.
